import { ADDRESS_ZERO, BIG_DECIMAL_ZERO, MINIMUM_USD_THRESHOLD_NEW_PAIRS } from '../../constants'
...
import { getTokenHourData, updateTokenHourData } from '../enitites/token-hour-data'
  ...
  if(pair){
    ...
    if (event.params.from == ADDRESS_ZERO) {
      ...
      if (mints.length != 0 && !isCompleteMint(mints[mints.length - 1])) {
        ...
        burn.feeTo = (mint !== null) ? mint.to : null
        burn.feeLiquidity = (mint !== null) ? mint.liquidity : null
...
export function onSync(event: SyncEvent): void {
  ...
  if(pair){
    ...
    if(token0 && token1){
      ...
      log.info(`Bundle ETH Price : {}`, [`${bundle.ethPrice}`])
      bundle.save()
...
export function onBurn(event: BurnEvent): void {
  ...
  if(pair){
    ...
    if(token0 && token1){
      ...
      const sender = burn.sender
      if(sender){
        const liquidityPosition = createLiquidityPosition(Address.fromBytes(sender), event.address, event.block)
        createLiquidityPositionSnapshot(liquidityPosition, event.block)
      }
      // update day data
      updateDayData(event)
...
export function onSwap(event: SwapEvent): void {
  ...
  if(pair){
    ...
    if(token0 && token1){
      ...
      if(amount0In == BigDecimal.fromString("0") && amount1Out == BigDecimal.fromString("0")){
        token0.selltxn = token0.selltxn.plus(BigInt.fromI32(1))
        token1.buytxn = token1.buytxn.plus(BigInt.fromI32(1));
      }else{
        token0.buytxn = token0.buytxn.plus(BigInt.fromI32(1))
        token1.selltxn = token1.selltxn.plus(BigInt.fromI32(1))
      }
      ...
      // Update buy and sell volume and transaction
      if(amount0In == BigDecimal.fromString("0") && amount1Out == BigDecimal.fromString("0")){
        token0DayData.selltxn = token0DayData.selltxn.plus(BigInt.fromI32(1))
        token1DayData.buytxn = token1DayData.buytxn.plus(BigInt.fromI32(1));
        token0DayData.sellVolume = token0DayData.sellVolume.plus(amount0Total)
        token1DayData.buyVolume = token1DayData.buyVolume.plus(amount1Total)
        token0HourData.selltxn = token0HourData.selltxn.plus(BigInt.fromI32(1))
        token1HourData.buytxn = token1HourData.buytxn.plus(BigInt.fromI32(1));
        token0HourData.sellVolume = token0HourData.sellVolume.plus(amount0Total)
        token1HourData.buyVolume = token1HourData.buyVolume.plus(amount1Total)
      }else{
        token0DayData.buytxn = token0DayData.buytxn.plus(BigInt.fromI32(1))
        token1DayData.selltxn = token1DayData.selltxn.plus(BigInt.fromI32(1))
        token0DayData.buyVolume = token0DayData.buyVolume.plus(amount0Total)
        token1DayData.sellVolume = token1DayData.sellVolume.plus(amount1Total)
        token0HourData.buytxn = token0HourData.buytxn.plus(BigInt.fromI32(1))
        token1HourData.selltxn = token1HourData.selltxn.plus(BigInt.fromI32(1))
        token0HourData.buyVolume = token0HourData.buyVolume.plus(amount0Total)
        token1HourData.sellVolume = token1HourData.sellVolume.plus(amount1Total)
      }

      // swap specific updating for token0
      token0HourData.volume = token0HourData.volume.plus(amount0Total)
      token0HourData.volumeETH = token0HourData.volumeETH.plus(amount0Total.times(token1.derivedETH as BigDecimal))
      ...
      token1HourData.volumeETH = token1HourData.volumeETH.plus(amount1Total.times(token1.derivedETH as BigDecimal))
