...
import { getFactory, getToken } from '.'

export function getPair(address: Address, block: ethereum.Block | null = null): Pair | null {
  let pair = Pair.load(address.toHex())

  if (pair === null) {
    const pairContract = PairContract.bind(address)

    const token0 = getToken(pairContract.token0(), block)

    if (token0 === null) {
      return null
    }

    const token1 = getToken(pairContract.token1(), block)
